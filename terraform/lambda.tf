...
# Existing database secret created by car-repair-db-infra.
# When the ARN is provided directly, this lookup is not required.
...
# JWT signing key secret managed by this stack.
...
# Random signing key for HS256 JWT generation.
...
resource "aws_secretsmanager_secret_version" "jwt_signing_key" {
  ...
  secret_string = jsonencode({
    secretKey = random_password.jwt_signing_key.result
  })
}

# CloudWatch log group for the authentication Lambda.
...
resource "aws_lambda_function" "auth" {
  ...
  # Normal AWS:
  #   Terraform can create its own execution role.
  #
  # AWS Academy:
  #   Reuse the pre-created LabRole.
  ...
  # Lambda runs in the same private network used by the application/RDS.
  vpc_config {
    subnet_ids = var.private_subnet_ids

    security_group_ids = [
      var.database_client_security_group_id
    ]
  }
  ...
  lifecycle {
    precondition {
      condition = (
        ...
        trimspace(var.execution_role_arn) != ""
      )
...
# -------------------------------------------------------------------
# Optional AWS API Gateway
#
# In the Academy deployment this is disabled because external traffic
# enters through:
#
# Internet -> NLB -> Kong -> Car Repair API -> Auth Lambda
#
# It remains available so the repository can support an architecture
# using AWS API Gateway when required.
# -------------------------------------------------------------------
